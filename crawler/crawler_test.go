...
func TestCrawlDepth0(t *testing.T) {
	...
	t.Run("without Tree", func(t *testing.T) {
		visited := newSeenMap()
		wg.Add(1)
		go crawl("https://g.org/", 0, ffetcher, nil, visited)
		wg.Wait()
	...
	t.Run("with Tree", func(t *testing.T) {
		...
		wg.Add(1)
		go crawl("https://g.org/", 0, ffetcher, rootNode, visited)
		wg.Wait()
		assert.Equal(t, expectedURLList, visited.urls)
...
func TestCrawlDepth1(t *testing.T) {
	...
	t.Run("without tree", func(t *testing.T) {
		visited := newSeenMap()
		wg.Add(1)
		go crawl("https://g.org/", 1, ffetcher, nil, visited)
		wg.Wait()
		assert.ElementsMatch(t, expectedURLList, visited.urls)
	...
	t.Run("with tree", func(t *testing.T) {
		...
		rootNode := tree.NewNode("https://g.org/")
		wg.Add(1)
		go crawl("https://g.org/", 1, ffetcher, rootNode, visited)
		wg.Wait()
		assert.ElementsMatch(t, expectedURLList, visited.urls)
	...
	t.Run("non existent URL", func(t *testing.T) {
		t.Run("without tree", func(t *testing.T) {
			visited := newSeenMap()
			wg.Add(1)
			go crawl("https://foo.org/", 1, ffetcher, nil, visited)
			wg.Wait()
...
func TestCrawlDepth2(t *testing.T) {
	...
	t.Run("without tree", func(t *testing.T) {
		visited := newSeenMap()
		wg.Add(1)
		go crawl("https://g.org/", 2, ffetcher, nil, visited)
		wg.Wait()
		assert.ElementsMatch(t, expectedURLs, visited.urls)
	})
	t.Run("with tree", func(t *testing.T) {
		visited := newSeenMap()
		rootNode := tree.NewNode("https://g.org/")
		wg.Add(1)
		go crawl("https://g.org/", 2, ffetcher, rootNode, visited)
		wg.Wait()
		assert.ElementsMatch(t, expectedURLs, visited.urls)
...
func TestCrawlDepth3(t *testing.T) {
	...
	t.Run("without tree", func(t *testing.T) {
		visited := newSeenMap()
		wg.Add(1)
		go crawl("https://g.org/", 3, ffetcher, nil, visited)
		wg.Wait()
		assert.ElementsMatch(t, depth3ExpectedURLs, visited.urls)
	})
	t.Run("with tree", func(t *testing.T) {
		visited := newSeenMap()
		rootNode := tree.NewNode("https://g.org/")
		wg.Add(1)
		go crawl("https://g.org/", 3, ffetcher, rootNode, visited)
		wg.Wait()
		assert.ElementsMatch(t, depth3ExpectedURLs, visited.urls)
		...
		t.Run("depth 4", func(t *testing.T) {
			...
			rootNode := tree.NewNode("https://g.org/")
			wg.Add(1)
			go crawl("https://g.org/", 4, ffetcher, rootNode, visited)
			wg.Wait()
			assert.ElementsMatch(t, depth3ExpectedURLs, visited.urls)
			assert.Equal(t, expectedTree, rootNode)
		})
		t.Run("depth 5", func(t *testing.T) {
			...
			rootNode := tree.NewNode("https://g.org/")
			wg.Add(1)
			go crawl("https://g.org/", 5, ffetcher, rootNode, visited)
			wg.Wait()
			assert.ElementsMatch(t, depth3ExpectedURLs, visited.urls)
			assert.Equal(t, expectedTree, rootNode)
		})
...
func TestActualWebsite(t *testing.T) {
	...
	go crawl("http://jarifibrahim.github.io", 3, fetchers.NewSimpleFetcher("http://jarifibrahim.github.io"), root, seenURLs)
	wg.Wait()
	assert.ElementsMatch(t, expectedURLs, seenURLs.urls)
}
